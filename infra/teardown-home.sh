#!/bin/bash
# home을 EC2와 AWS에서 완전히 지운다.
#
#   ./teardown-home.sh --dry-run   지울 것만 보여준다 (기본값)
#   ./teardown-home.sh --yes       실제로 지운다
#
...
HOME_SG="sg-0b95c6e321aeeda8d" # home 전용 SG. tium의 sg-05d78c3cfeb18ea4c 가 아니다
...
if [ "$DRY" = "1" ]; then
  ...
  echo " home을 완전히 삭제합니다"
  read -rp " 확인을 위해 home 을 입력하세요: " confirm
  [ "$confirm" = "home" ] || { echo " 취소했습니다."; exit 1; }
fi
echo "════════════════════════════════════════════"

echo
echo "[1] 컨테이너"
if docker ps -a --format '{{.Names}}' | grep -qx home; then
  assert_ours home && { run docker stop home; run docker rm home; }
else
  echo "  없음"
fi
...
echo
echo "[2] 이미지"
IMAGES=$(docker images --format '{{.Repository}}:{{.Tag}}' | grep '/home:' || true)
if [ -n "$IMAGES" ]; then
...
echo
echo "[3] 네트워크 (home-network)"
if docker network inspect home-network >/dev/null 2>&1; then
  run docker network rm home-network
else
  echo "  없음"
fi
...
echo
echo "[4] EC2 파일"
for f in "$INFRA_DIR/home.env" "$INFRA_DIR/docker-compose.home.yml" "$INFRA_DIR/deploy-home.sh"; do
  [ -f "$f" ] && run rm -f "$f" || echo "  없음: $f"
done

echo
echo "[5] SSM 파라미터 (/home/prod/*)"
PARAMS=$(aws ssm get-parameters-by-path --path /home/prod/ --region "$REGION" \
  --query 'Parameters[].Name' --output text 2>/dev/null | tr '\t' '\n' || true)
if [ -n "$PARAMS" ]; then
  for p in $PARAMS; do
    case "$p" in
      /home/prod/*) run aws ssm delete-parameter --name "$p" --region "$REGION" ;;
      *) echo "  !! '$p' 은 /home/prod/ 밖입니다. 건너뜁니다." ;;
    esac
  done
else
  echo "  없음"
...
echo
echo "[6] 데이터베이스 (home DB + home 계정)"
echo "  RDS 인스턴스는 tium과 공유하므로 인스턴스는 건드리지 않습니다."
echo "  DB와 계정만 지웁니다."
if [ "$DRY" = "1" ]; then
  echo "  [dry-run] DROP DATABASE home; DROP USER 'home'@'%';"
else
  ...
  P=$(aws ssm get-parameter --name /tium/prod/MYSQL_PASSWORD --with-decryption --region "$REGION" \
       --query Parameter.Value --output text)
  mysql -h "$H" -u "$U" -p"$P" -e \
    "DROP DATABASE IF EXISTS home; DROP USER IF EXISTS 'home'@'%';" 2>&1 \
    | grep -v "Using a password" | sed 's/^/    /'
  echo "    완료 (tium / handari DB는 그대로입니다)"
fi

echo
echo "[7] 전용 보안그룹 (home-sg $HOME_SG)"
# tium SG는 규칙 쿼터가 꽉 차서(prefix list 규칙 1개 = 55개로 계산) home은 전용 SG를 썼다.
...
if echo "$CURRENT" | grep -q "$HOME_SG"; then
...
if aws ec2 describe-security-groups --region "$REGION" --group-ids "$HOME_SG" >/dev/null 2>&1; then
  run aws ec2 delete-security-group --region "$REGION" --group-id "$HOME_SG"
else
  echo "  없음"
fi
...
echo
echo "[8] Route 53 레코드 (home.devckm.kr)"
...
REC=$(aws route53 list-resource-record-sets --hosted-zone-id "$ZONE" \
  --query "ResourceRecordSets[?contains(Name,'home')]" \
...
echo
echo "[9] CloudFront 배포 (home.devckm.kr)"
DIST=$(aws cloudfront list-distributions \
  --query "DistributionList.Items[?contains(Aliases.Items[0] || '', 'home')].Id" --output text 2>/dev/null || true)
if [ -n "$DIST" ]; then
  echo "  배포 ID: $DIST"
  if [ "$DRY" = "1" ]; then
    ...
    aws cloudfront get-distribution-config --id "$DIST" --query DistributionConfig > /tmp/home-cf-off.json
    python3 -c "
import json; d=json.load(open('/tmp/home-cf-off.json')); d['Enabled']=False
json.dump(d,open('/tmp/home-cf-off.json','w'))"
    aws cloudfront update-distribution --id "$DIST" --distribution-config file:///tmp/home-cf-off.json --if-match "$ETAG" >/dev/null
...
echo
echo "[10] ACM 인증서 (home.devckm.kr)"
CERT=$(aws acm list-certificates --region us-east-1 \
  --query "CertificateSummaryList[?DomainName=='home.devckm.kr'].CertificateArn" --output text 2>/dev/null || true)
if [ -n "$CERT" ]; then
...
echo "════════════════════════════════════════════"
cat <<'MANUAL'
  1. GitHub      wally-cho/home 리포 삭제 또는 archive
                 Settings → Secrets 에 등록한 8개도 함께 사라집니다
  2. Docker Hub  chokyumin/home 리포지토리 삭제
  3. 카카오      developers.kakao.com 에서 home 앱 삭제
  4. 로컬        ~/github/home 디렉터리, ~/.zshrc 의 devw alias
